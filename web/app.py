...
def criar_reclamacao():
    ...
    texto, erro = _validate_complaint_text(
        dados["reclamacao"]
    )
...
@app.route("/api/reclamacoes/<int:codigo>",
           methods=["GET"])
...
def atualizar_reclamacao(codigo):
    ...
    if not dados or not dados.get("reclamacao"):
        return jsonify({
            "erro":
                "Campo 'reclamacao' é obrigatório."
        }), 400

    novo_texto, erro = (
        _validate_complaint_text(
            dados["reclamacao"]
        )
    )
...
def remover_reclamacao(codigo):
    ...
    try:
        consulta = (
            "DELETE FROM Reclamações "
            "WHERE codigo = %s"
        )

        linhas = excluirBancoDados(
            conexao,
            consulta,
            [codigo],
        )
