...
async function carregarProdutos() {
    showSkeleton();
    try {
        const resProdutos = await fetch(API_BASE);
        ...
        if (statTotal) statTotal.textContent = produtos.length ?? '--';
        if (statBar) statBar.style.width = Math.min((produtos.length || 0) * 5, 100) + '%';
...
// ===== MODAL FUNCTIONS =====
function openEdit(id, nome, categoria, quantidade, preco, fornecedor) {
